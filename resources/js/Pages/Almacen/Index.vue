        ...
        <div v-if="registers.length> 0" class="mt-4">
            ...
            <li class="mb-2" v-for="almacen in registers"> 
              <div class="w-full px-4 pb-3 border-solid border-b-2 border-gray-700">
                <div class="w-full h-24 grid grid-cols-8 grid-rows-2 gap-x-1 place-items-center">
                  ...
                  <div class="w-full h-full place-items-center col-span-2 row-span-2 grid grid-cols-2 rounded border-2 border-dashed border-black">
                  ...
                  <Link class="w-full flex items-center justify-center h-5/6 mb-1 text-sm ml-3 text-gray-900 font-bold p-1 rounded border-2 border-solid border-black" :href="route('almacen.edit', almacen.id)">EDITAR</Link>
                  <button class="w-full flex items-center justify-center h-5/6 mt-1 text-sm ml-3 text-gray-900 font-bold p-1 rounded border-2 border-solid border-black" @click="deleteCategory(almacen.id)">BORRAR</button>
                </div>
              </div>
            </li>
          </ul>
          <div v-else>
            <div class="w-full px-4 py-3 border-solid border-y-2 border-gray-700">
              <div class="w-full py-4 grid place-items-center text-xl">No se encontró ningún almacén</div>
            </div>
          </div>
        </div>
