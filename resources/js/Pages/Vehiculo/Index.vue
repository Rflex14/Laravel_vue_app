    ...
    <div class="py-12 ">
      <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="mt-4">
            ...
            <li class="mb-2" v-for="vehiculo in registers"> 
              <div class="w-full px-4 py-3 rounded-lg border-solid border-2 border-gray-900">
                <div class="w-full pl-4 grid grid-cols-6 grid-rows-2 place-content-evenly">
                  ...
                  <div v-if="!(vehiculo.persona_id === null)" class="w-full place-items-center col-span-2 row-span-2 grid grid-cols-2 rounded border border-solid border-black bg-yellow-400 pt-6 pb-6">
                  ...
                  <div v-else class="w-full place-items-center col-span-2 row-span-2 grid grid-cols-2 rounded border border-solid border-black bg-yellow-400 pt-6 pb-6">
                  ...
                  <Link class="mb-1 text-sm ml-3 leading-6 text-gray-900 font-bold text-center p-1 bg-red-500 rounded border border-solid border-black" :href="route('vehiculo.edit', vehiculo.id)">EDITAR</Link>
                  <button class="mt-1 text-sm ml-3 leading-6 text-gray-900 font-bold text-center p-1 bg-red-500 rounded border border-solid border-black" @click="deleteCategory(vehiculo.id)">BORRAR</button>
                </div>
              </div>
            </li>
          </ul>
          <div v-else>
            <div class="w-full px-4 py-3 rounded-lg border-solid border-2 border-gray-900">
              <div class="w-full py-4 grid place-items-center text-xl">No se encontro ningun vehiculo</div>
            </div>
          </div>
        </div>  
