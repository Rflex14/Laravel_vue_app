...
const propietario = computed(() => {
  if (tipo_lugar.value === 'unidad') {
    const unidad = props.unidades.find(unidad => unidad.id === props.form.lugar_id);
    if(unidad) {
      return unidad.empresa_id ? unidad.empresa : unidad.persona;
    }
  } else if (tipo_lugar.value === 'almacen') {
    const almacen = props.almacenes.find(almacen => almacen.id === props.form.lugar_id);
    if(almacen) {
      return almacen.empresa_id ? almacen.empresa : almacen.persona;
    }
  } else if (tipo_lugar.value === 'vehiculo') {
    const vehiculo = props.vehiculos.find(vehiculo => vehiculo.id === props.form.lugar_id)
    if(vehiculo) {
      return vehiculo.empresa_id ? vehiculo.empresa : vehiculo.persona;
    }
  }
  return null;
})
...
<template>
  <FormSection @submitted="$emit('submit')">
    ...
    <template #form>
      ...
      <div class="col-span-6 sm:col-span-6">
        <InputLabel for="dia_mes_año" value="Dia-Mes-Año"/>
        <TextInput id="dia_mes_año" type="date" v-model:="form.dia_mes_año" class="mt-1 block w-full"/>
        <InputError :message="$page.props.errors.dia_mes_año" class="mt-2" />
      </div>
      <div class="col-span-6 sm:col-span-6">
      ...
      <div v-if="tipo_lugar !== '' && form.lugar_id !== ''" class="col-span-6 sm:col-span-6">
        ...
        <select name="producto_id" id="producto_id" v-model:="form.producto_id" class="mt-1 block w-full">
          ...
          <option v-for="producto in productosFiltrados" :value="producto.id">{{ producto.nombre }}</option>
        </select>
      ...
      <div class="col-span-6 sm:col-span-6">
        ...
        <TextInput id="cantidad_afectada" type="number" v-model:="form.cantidad_afectada" class="mt-1 block w-full"/>
        <InputError :message="$page.props.errors.cantidad_afectada" class="mt-2" />
      </div>
      <div class="col-span-6 sm:col-span-6">
