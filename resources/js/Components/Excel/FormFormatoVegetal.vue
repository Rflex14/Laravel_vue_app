...
const hectareasTotales = computed(() => {
  const unidad = props.unidades.find(unidad => unidad.id === props.form.unidad_id)
  if(unidad) {
    let hecT = unidad.hectareasActivas + unidad.hectareasInactivas;
    props.form.hectareas_totales = hecT;
    return hecT;
  }
  return null;
})
...
<template>
  <FormSection @submitted="$emit('submit')">
    <template #title>
      Registrar Inspección de Predio en el Formato Protección Vegetal
    </template>
    ...
    <template #form>
      ...
      <div class="col-span-6 sm:col-span-6">
        <InputLabel for="numero_acta" value="Número de acta"/>
      ...
      <div v-else class="col-span-6 sm:col-span-6">
        <InputLabel for="rubro_id" value="Rubro o Producto"/>
        <div id="rubro_id" name="rubro_id" class="block mt-1 h-12 pl-2 w-full flex items-center border border-solid border-gray-300 focus:ring-indigo-500 rounded-md shadow-sm">Seleccione un predio primero</div>
      </div>
      <div class="col-span-6 sm:col-span-6">
        <InputLabel for="hectareas_totales" value="Hectareas Totales"/>
        <div v-if="hectareasTotales !== null" id="hectareas_totales" name="hectareas_totales" class="block mt-1 h-12 pl-2 w-full flex items-center border border-solid border-gray-300 focus:ring-indigo-500 rounded-md shadow-sm">{{hectareasTotales}}</div>
        <div v-else id="hectareas_totales" name="hectareas_totales" class="block mt-1 h-12 pl-2 w-full flex items-center border border-solid border-gray-300 focus:ring-indigo-500 rounded-md shadow-sm">Seleccione un predio primero</div>
      </div>
      <div class="col-span-6 sm:col-span-6">
        <InputLabel for="hectareas_sembradas" value="Hectareas Sembradas"/>
        <div v-if="propietario === null || form.rubro_id === ''" id="propietario" name="propietario" class="block mt-1 h-12 pl-2 w-full flex items-center border border-solid border-gray-300 focus:ring-indigo-500 rounded-md shadow-sm">Seleccione un predio y un producto primero</div>
        <TextInput v-else id="hectareas_sembradas" type="number" v-model:="form.hectareas_sembradas" class="mt-1 block w-full"/>
        <InputError :message="$page.props.errors.hectareas_sembradas" class="mt-2" />
      </div>
      <div class="col-span-6 sm:col-span-6">
        <InputLabel for="hectareas_atendidas" value="Hectáreas atendidas"/>
      ...
      <div class="col-span-6 sm:col-span-6">
        <InputLabel for="hectareas_afectadas" value="Hectáreas afectadas"/>
